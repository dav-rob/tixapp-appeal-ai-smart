...
# TixApp Android Build Wrapper Script
...
if [ -n "$ANDROID_HOME" ] && command -v adb &> /dev/null; then
    ...
    exec "$(dirname "$0")/android-build-run.sh" "$@"
else
    ...
    exec bash -i -c "source ~/.bashrc 2>/dev/null || source ~/.zshrc 2>/dev/null || true; $(dirname "$0")/android-build-run.sh $*"
fi